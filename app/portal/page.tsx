...
import { getStudentPortalData } from "@/lib/queries/portal";
    ...
    <>
        ...
        {data.fees.items.length === 0 ? (
          ...
          data.fees.items.map((fee: (typeof data.fees.items)[number]) => (
            <div className="panel-row" key={fee.id}>
              <span className="panel-row-title">{fee.feeStructure.name}</span>
              <span>
                <span
      ...
      <div className="panel">
        ...
        {data.attendance.recent.length === 0 ? (
          ...
          data.attendance.recent.map(
            (a: (typeof data.attendance.recent)[number]) => (
